...
export const getModules = async ({ pages, limit, search = "" }) => {
  try {
    const response = await axios.get(
      `/exam/module?search=${search}&limit=${limit}&pages=${pages}`
    );
    ...
    selector: (row) => (
      <div className="d-flex">
        <UncontrolledButtonDropdown>
          ...
          <DropdownMenu>
            <DropdownItem href={`/ujian/modul/pratinjau-soal/${row.module_code}`} tag="a">
              <img
            ...
            <DropdownItem href={`/ujian/modul/edit/${row.module_code}`} tag="a">
