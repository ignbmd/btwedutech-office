...
export const getSubQuestionCategory = async () => {
  const response = await axios.get(`/exam-cpns/sub-question-category`);
...
const deleteQuestion = async (id) => {
  try {
    const url = `/exam-cpns/sub-question-category/${id}`;
...
export const columns = [
  ...
  },
  {
    ...
    sortable: false,
    style: { height: "unset !important" },
    selector: ({ passing_grade }) => (passing_grade ? passing_grade : "-"),
  },
  {
    name: "Program",
    wrap: true,
  ...
  },
  {
    ...
    selector: (row) => (
      <UncontrolledButtonDropdown>
        ...
        <DropdownMenu>
          <DropdownItem
            href={`/ujian-cpns/sub-kategori-soal/edit/${row.id}`}
            tag="a"
          >
