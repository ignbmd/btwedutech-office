...
import { Edit, Trash2, File } from "react-feather";
import { initialDatatables } from "../api/datatables/study-material-data";
...
export const getStudyMaterial = async () => {
  const response = await axios.get("/exam/study-material");
...
export const columns = [
  ...
  },
  {
    ...
    sortable: false,
    style: { height: "unset !important" },
    selector: ({ program }) => <div className="text-uppercase">{program}</div>,
  },
  {
    name: "Kategori Soal",
    wrap: true,
    ...
    sortable: false,
    style: { height: "unset !important" },
    selector: ({ question_categories: { description } }) => (
      <div>{description}</div>
    ),
  },
  {
  ...
  },
  {
    ...
    sortable: false,
    style: { height: "unset !important" },
    selector: ({ description }) => <CKEditorParser mString={description} />,
  },
  {
    name: "Status",
    wrap: true,
    ...
    selector: (row) => (
      <div className="d-flex">
        <UncontrolledButtonDropdown>
          ...
          <DropdownMenu>
            <DropdownItem
              href={`/ujian/materi-belajar/dokumen/${row.id}`}
              tag="a"
            >
              <File size={15} className="mr-50" /> Lihat Dokumen Materi Belajar
            </DropdownItem>
            <DropdownItem href={`/ujian/materi-belajar/edit/${row.id}`} tag="a">
