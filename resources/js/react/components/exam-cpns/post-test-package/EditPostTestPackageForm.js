...
const FormSchema = yup.object().shape({
  ...
  duration: yup
    ...
    .max(90, "Tidak boleh lebih dari 90")
...
const EditPreTestPackageForm = () => {
  const [postTestPackage, setPostTestPackage] = useState(null);
  ...
  const fetchPackage = async (id) => {
    try {
      const response = await axios.get(`/api/exam-cpns/post-test-package/${id}`);
  ...
  const onSubmit = async () => {
    try {
      ...
      const payload = {
        ...
        legacy_task_id: postTestPackage.legacy_task_id,
      ...
      const response = await axios.post(`/api/exam-cpns/post-test-package/${id}`, {
        data: payload,
        cancelToken: source.token,
      });
  ...
  const showSuccessToast = () => {
    toastr.success(`Paket post-test berhasil diperbarui`, `Berhasil`, {
      timeOut: 2000,
      closeButton: true,
      tapToDismiss: true,
      preventDuplicates: true,
      onHidden() {
        window.location.href = `/ujian-cpns/paket-post-test`;
      },
    });
  };
  ...
  useEffect(() => {
    if (!postTestPackage || !instructions) return;
    if(isAlreadyLoadinstructions.current === false) {
      setValue(
        "instruction",
        instructions?.find(
          (instruction) => instruction.id === postTestPackage.instructions_id
        )
      );
    }
    if(instructions.length > 0 && postTestPackage.instructions !== null) {
      isAlreadyLoadinstructions.current = true;
    }
    setValue("tags", postTestPackage.tags);
    setIsDataLoaded(true);
  }, [postTestPackage, instructions]);
