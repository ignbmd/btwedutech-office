...
import {
  columns,
  getPostTestPackages,
} from "../../../data/exam-cpns/post-test-package-table.js";


import TableLoader from "../../core/skeleton-loader/TableLoader";

const PostTestPackageTable = () => {
    ...
    setFilteredData(
      data.filter((item) => {
        ...
        const matchModuleCode = item?.modules?.module_code
          ?.toLowerCase()
          ?.includes(key);
        const anyMatch = [matchTitle, matchModuleCode].some(Boolean);
        return anyMatch;
      })
    );
  };
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await getPostTestPackages();
    ...
    <Card>
      <Row className="justify-content-end mx-0">
        <Col
          ...
          sm="12"
        >
          <a href="/ujian-cpns/paket-post-test/create" className="btn btn-primary">
            <Plus size={14} /> Tambah Paket Post-Test
          </a>
        </Col>
      </Row>
      <hr />
...
export default PostTestPackageTable;
