...
import {
  columns,
  data,
  getTryoutCodeCategories,
} from "../../../data/tryout-code-category-table-cpns";
...
const TryoutCodeTable = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await getTryoutCodeCategories();
  ...
  const handleFilter = (e) => {
    ...
    setFilteredData(
      data.filter((item) => {
        ...
        const matchTryoutCodeCategoryName = item?.name
          .toLowerCase()
          .includes(key);
        const anyMatch = [matchTryoutCodeCategoryName].some(Boolean);
        return anyMatch;
      })
    );
  };
  ...
  return (
    <Card>
      <Row className="justify-content-end mx-0">
        ...
        >
          <a
            href="/ujian-cpns/kategori-tryout-kode/create"
            className="btn btn-primary"
          >
            <Plus size={14} /> Buat Kategori Tryout Kode
          </a>
        </Col>
      </Row>
      <hr />
