...
const CreateEditTrialModuleForm = ({ type }) => {
  ...
  const createTrialModule = async () => {
    try {
      ...
      const response = await axios.post("/api/exam-cpns/trial-module", payload);
      ...
      window.location.href = "/ujian-cpns/coba-modul";
    } catch (error) {
      console.error(error);
      showToast({
        type: "error",
  ...
  const fetchTrialModule = async (id) => {
    try {
      const response = await axios.get(`/api/exam-cpns/trial-module/${id}`, {
        cancelToken: source.token,
      });
