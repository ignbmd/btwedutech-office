...
const CreateEditSubQuestionCategoryForm = ({ type }) => {
  ...
  const getCurrentInstruction = async () => {
    try {
      ...
      const response = await axios.get(
        `/exam-cpns/question-category/detail/${id}`,
        {
  ...
  const redirectToIndex = () => {
    window.location.href = "/ujian-cpns/kategori-soal";
  };
  ...
  const processForm = async () => {
    ...
    try {
      if (type === "edit") {
        const id = getLastSegment();
        await axios.put(`/exam-cpns/question-category/${id}`, payload);
      } else {
        await axios.post("/exam-cpns/question-category/create", payload);
      }
