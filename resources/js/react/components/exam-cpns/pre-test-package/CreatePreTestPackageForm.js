...
const CreatePreTestPackageForm = () => {
  ...
  const fetchInstructions = async (program) => {
    try {
      const response = await axios.get(`/api/exam-cpns/instruction/${program}`, {
  ...
  const createPreTestPackage = async (payload) => {
    try {
      const response = await axios.post(`/api/exam-cpns/pre-test-package/bulk`, {
        data: payload,
        cancelToken: source.token,
      });
      return {
  ...
  const onSubmit = async () => {
    try {
      ...
      const response = await createPreTestPackage(payload);
  ...
  const showSuccessToast = () => {
    toastr.success(`Paket pre-test berhasil ditambah`, `Berhasil`, {
      timeOut: 2000,
      closeButton: true,
      tapToDismiss: true,
...
export default CreatePreTestPackageForm;
