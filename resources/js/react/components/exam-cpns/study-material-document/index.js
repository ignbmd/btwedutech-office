...
import {
  columns,
  getStudyMaterialDocument,
} from "../../../data/study-material-table-document-cpns";

import TableLoader from "../../core/skeleton-loader/TableLoader";
import { getLastSegment } from "../../../utility/Utils";

const studyMaterialId = getLastSegment();
const StudyMaterialDocumentTable = () => {
  ...
  const handleFilter = (e) => {
    ...
    setFilteredData(
      data.filter((item) => {
        ...
        const anyMatch = [matchName, matchType].some(Boolean);
        return anyMatch;
      })
    );
  };
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await getStudyMaterialDocument();
  ...
  return (
    <Card>
      <Row className="justify-content-end mx-0">
        ...
        >
          <a
            href={`/ujian-cpns/materi-belajar/dokumen/${studyMaterialId}/create`}
            className="btn btn-primary"
          >
            <Plus size={14} /> Tambah Dokumen Materi Belajar
          </a>
        </Col>
      </Row>
      <hr />
...
export default StudyMaterialDocumentTable;
