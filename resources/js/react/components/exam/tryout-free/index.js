...
import { columns, getTryoutFree } from "../../../data/tryout-free-table";
import TableLoader from "../../core/skeleton-loader/TableLoader";

const TryoutFreeTable = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await getTryoutFree();
  ...
  const handleFilter = (e) => {
    ...
    setFilteredData(
      data.filter((item) => {
        ...
        const anyMatch = [matchTitle, matchBranchCode].some(Boolean);
        return anyMatch;
      })
    );
  };
    ...
    <Card>
      <Row className="justify-content-end mx-0">
        <Col
          ...
          sm="12"
        >
          <a href="/ujian/tryout-gratis/create" className="btn btn-primary">
            <Plus size={14} /> Buat Tryout Gratis
          </a>
        </Col>
      </Row>
      <hr />
...
export default TryoutFreeTable;
