...
const CreateEditTryoutPremiumForm = ({ type }) => {
  ...
  useEffect(() => {
    ...
    if (type === "edit" && isLoading) {
      ...
      if (tryout?.product_code) {
        setValue(
          "product",
          products.find((value) => tryout?.product_code === value.product_code)
        );
      }
      if (tryout?.instructions) {
        setValue("instruction", {
          id: tryout?.instructions?.id,
          title: tryout?.instructions?.title,
        });
      }
      if (tryout?.modules) {
        setValue("sections.0.modul", {
          id: tryout?.modules?.id,
          name: tryout?.modules?.name,
        });
      }
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    if (!program.slug && type !== "edit") return;
    if (!program.slug && type === "edit" && !tryout) return;

    if (!program.slug && type === "edit" && tryout) {
      setValue(
        "program",
        programs?.find((program) => program?.slug === tryout?.program)
      );
    }
    setValue("product", "");
    setValue("instruction", "");
