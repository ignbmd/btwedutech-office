...
const CreatePackageForm = () => {
  ...
  const createPremiumPackage = async (payload) => {
    try {
      const response = await axios.post(`/api/exam/assessment-package/bulk`, {
        data: payload,
        cancelToken: source.token,
      });
      return {
  ...
  const showSuccessToast = () => {
    toastr.success(`Paket berhasil ditambah`, `Berhasil`, {
      timeOut: 2000,
      closeButton: true,
      tapToDismiss: true,
  ...
  const showErrorToast = () => {
    toastr.error(
      `Paket gagal ditambah, silakan coba lagi nanti`,
      `Terjadi kesalahan`,
      {
        timeOut: 2000,
        closeButton: true,
