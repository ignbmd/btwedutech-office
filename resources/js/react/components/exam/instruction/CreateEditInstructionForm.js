...
const CreateEditInstructionForm = ({ type }) => {
  ...
  const getCurrentInstruction = async () => {
    try {
      ...
      const response = await axios.get(`/exam/instruction/detail/${id}`, {
  ...
  const redirectToIndex = () => {
    window.location.href = "/ujian/instruksi";
  };
  ...
  const processForm = async () => {
    ...
    try {
      if (type === "edit") {
        const id = getLastSegment();
        await axios.put(`/exam/instruction/${id}`, payload);
      } else {
        await axios.post("/exam/instruction/create", payload);
      }
