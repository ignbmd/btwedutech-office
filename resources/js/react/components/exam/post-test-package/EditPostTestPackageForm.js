...
const EditPreTestPackageForm = () => {
  ...
  const fetchPackage = async (id) => {
    try {
      const response = await axios.get(`/api/exam/post-test-package/${id}`);
  ...
  const fetchInstructions = async (program) => {
    try {
      const response = await axios.get(
        `/api/exam/instruction/?program=${program}`,
  ...
  const onSubmit = async () => {
    try {
      ...
      const response = await axios.post(`/api/exam/post-test-package/${id}`, {
        data: payload,
        cancelToken: source.token,
      });
