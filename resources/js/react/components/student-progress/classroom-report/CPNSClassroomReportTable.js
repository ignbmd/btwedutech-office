...
const CPNSClassroomReportTable = () => {
  const {
    selectedClassroom,
    selectedUkaType,
    selectedUkaModule,
  ...
  const targetTemplate = (data) => {
    return `${data?.student_target_cpns?.school_name} - ${data?.student_target_cpns?.major_name}`;
  };

  const passPercentageTemplate = (data) => {
    return `${data?.student_target_cpns?.current_target_percent_score}%`;
  };
  ...
  const handleDownloadStudentProgressReport = async (selectedReports) => {
    ...
    try {
      ...
      if (studentIDs.length == 1) {
        await streamStudentMultipleUkaReport(studentIDs[0]);
        return;
      }
  ...
  const streamStudentMultipleUkaReport = async (studentID) => {
      ...
      <CardBody>
        <PrimeReactProvider>
          <DataTable
            ...
            globalFilterFields={[
              "name",
              "student_target_cpns.school_name",
              "student_target_cpns.major_name",
              "student_target_cpns.target_score",
              "summary.owned",
              "summary.done",
              ...
              "student_target_cpns.current_target_percent_score",
            ]}
          >
            <Column
            ...
            ></Column>
            <Column
              field="student_target_cpns.target_score"
...
export default CPNSClassroomReportTable;
