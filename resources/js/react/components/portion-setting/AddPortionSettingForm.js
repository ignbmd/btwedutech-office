...
const AddPortionSettingForm = () => {
  ...
  async function onSubmit() {
    trigger();
    // Define request url
    const url = "/api/portion-setting/create"; // url post api
  ...
  useEffect(() => {
    fetchProduct();
    setValue("amount_type", portionSettingAmountTypes[0]);
    setSelectedAmountType(portionSettingAmountTypes[0].value);
    return () => {
      clearInterval();
      clearTimeout();
    };
...
export default AddPortionSettingForm;
