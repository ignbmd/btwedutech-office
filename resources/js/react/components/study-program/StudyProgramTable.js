...
import { columns, getStudyProgram } from "../../data/study-program-table";
...
const StudyProgramTable = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await getStudyProgram();
  ...
  const handleFilter = (e) => {
    ...
    setFilteredData(
      data.filter((item) => {
        ...
        const matchSchoolName = item?.school?.name.toLowerCase().includes(key);
        const anyMatch = [matchName, matchSchoolName].some(Boolean);
        return anyMatch;
      })
    );
  };
  ...
  return (
    <Card>
      <Row className="justify-content-end mx-0">
        ...
        >
          <a
            href="/peta-persaingan/program-studi/tambah"
            className="btn btn-primary"
          >
            <Plus size={14} /> Tambah Program Studi
          </a>
        </Col>
      </Row>
      <hr />
