...
const CentralDebtAndReceivableDetail = ({
  title,
  type,
  accountId,
  triggered,
  ...
  const getFormDataValues = () => {
    ...
    const payload = {
      ...
      status: type == "pay" ? "COMPLETED" : "OPEN",
      from: "CENTRAL",
      log_type: type == "pay" ? "PAY" : "BILL",
      payment_method:
  ...
  const onSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        `/api/finance/pay-and-bill/central/create/${type}`,
        fd,
        {
          headers: {
      ...
      <CardBody className={classnames(isSubmitting && "block-content")}>
        <Col md={6} className="pl-0 mt-2">
          ...
          <h5 className="font-weight-bolder">
            Proses {type == "pay" ? "Pembayaran" : "Penagihan"}
          </h5>
...
CentralDebtAndReceivableDetail.propTypes = {
  title: PropTypes.oneOfType([PropTypes.string, PropTypes.element]),
  type: PropTypes.oneOf(["pay", "bill"]),
};

export default CentralDebtAndReceivableDetail;
