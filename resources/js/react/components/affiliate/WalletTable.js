...
import { columns, getWalletsByAffiliateID } from "../../data/wallet-table";
...
const WalletTable = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await getWalletsByAffiliateID(affiliateID);
  ...
  const handleFilter = (e) => {
    ...
    setFilteredData(
      data.filter((item) => {
        ...
        const matchNo = item?.no?.toLowerCase().includes(key);
        const matchAmount = item?.amount?.toLowerCase().includes(key);
        const anyMatch = [matchNo, matchAmount].some(Boolean);
        return anyMatch;
      })
    );
  };
  ...
  return (
    <WalletTableContext.Provider value={contextProviderValue}>
      <Card>
