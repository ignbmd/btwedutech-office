...
<div id="sso_id" class="d-none">{{ $auth_user_id }}</div>
<div id="is_admin" class="d-none">{{UserRole::isAdmin() ? '1' : '0'}}</div>
<div id="is_mentor" class="d-none">{{UserRole::isMentor() ? '1' : '0'}}</div>
<div id="is_branch_admin" class="d-none">{{UserRole::isBranchAdmin() ? '1' : '0'}}</div>
<div id="user_branch_code" class="d-none">{{json_encode($auth_user_branch_code)}}</div>
<div id="is_user_has_multiple_branch_code" class="d-none">{{$is_user_has_multiple_branch_code ? '1' : '0'}}</div>
<div id="can_show_all_classroom" class="d-none">{{ $canShowAllClassroom ? "1" : "0" }}</div>
    ...
    <div class="modal-dialog sidebar-sm">
      <form class="add-new-record modal-content pt-0">
        ...
        <div class="modal-body flex-grow-1">
          <x-learning.class-body-form type="add" :utype="$userType" :branches="$branches" :products="$products" />
          <div class="mt-3">
...
<script src="{{ asset(mix('js/scripts/learning/class-branch-datatables.js')) }}"></script>
<script>
  $("#branch_code").select2();
</script>
@endsection
