...
@section('title', "Soal " . $stage->type . " Stage " . $stage->stage . " Level " . $stage->level)
              ...
              <div class="form-group col-4">
                ...
                <select name="question_category_id" id="question_category_id" class="form-control select2" @if($isMentor) required @endif>
                  ...
                  @foreach($questionCategories as $questionCategory)
                    <option value="{{ $questionCategory->id }}" @if(request()->get('question_category_id') == $questionCategory->id) selected @endif>{{ $questionCategory->category }}</option>
                  @endforeach
                </select>
              </div>
              <div class="form-group col-4">
                  ...
                  <td>
                    <div class="d-inline-flex">
                      ...
                      <div class="dropdown-menu dropdown-menu-right">
                        ...
                        <a class="dropdown-item w-100" href="/soal-stages/{{ $type }}/{{ $stage->_id }}/soal/{{ $data->id }}/komentar">
...
<script>
  window.addEventListener("load", function() {
    ...
    $(document).on('click', '.question-explanation', function() {
      ...
      const url = `${window.location.origin}/soal-stages/${stageType}/${stageID}/soal/${questionID}/pembahasan?qid=${encryptedText}&mid=${encryptedUserIdText}&mnm=${encryptedUserNameText}&track=comment`;
      window.open(url, '_blank');
    })
  });
</script>
