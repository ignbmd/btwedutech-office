...
@section('title', 'Edit Atur Besaran Komisi Per Mitra')
    ...
    <div class="card-body">
      <div class="row">
        <div class="col-12 col-md-6">
          <form action="{{ route('pengaturan-porsi-per-mitra.update', $affiliatePortion->id) }}" method="POST">
            @csrf
            @method("PUT")
              ...
              >
                ...
                @foreach($products as $product)
                  <option
                    ...
                    {{ json_encode($productData) == json_encode($product) ? 'selected' : ''}}
                  >
            ...
            <div class="form-group">
              ...
              >
                <option
                  value="PERCENT"
                  @if($affiliatePortion->amount_type === "PERCENT") selected @endif
                >
                  PERCENT
                </option>
                <option
                  value="FIXED"
                  @if($affiliatePortion->amount_type === "FIXED") selected @endif
                >
                  FIXED
                </option>
              </select>
            ...
            <div class="form-group">
              ...
              >
                ...
                <option value="UPLINE_FEE" @if($affiliatePortion->type === "UPLINE_FEE") selected @endif>UPLINE</option>
                <option value="NORMAL_FEE" @if($affiliatePortion->type === "NORMAL_FEE") selected @endif>
                  DOWNLINE
                </option>
              </select>
              @error("type")
...
<script>
  (function (window, document, $) {
    ...
    let affiliatePortion = @json($affiliatePortion);
    ...
    function setInitialPortionAmountFormValue() {
      ...
      amountInputElement.val(affiliatePortion.amount);
