...
<body>
    ...
    <table class="header">
        <tr>
            <td class="title">
                Laporan Hasil Perkembangan Belajar {{$program_title ?? ''}}
            </td>
    ...
    <div class="content">
        ...
        <table class="info">
            ...
            <tr>
                <td>Program</td>
                <td>: {{strtoupper($program_title) ?? ''}}</td>
            </tr>
            <tr>
        ...
        <table class="module custom-table">
            ...
            <tr style="font-size: 1.5rem">
                ...
                <th>{{number_format($report->passed_percent, 2) ?? 0}}%</th>
            </tr>
        </table>
        ...
        <table class="items custom-table" style="margin-top: 20px">
            @if (count($report->report ?? []) > 0)
                @if ($program != 'tka-campuran')
                    @php
                        $keys = array_keys((array)$report->report[0]->score_values ?? []);
                    @endphp
                    <tr class="head">
                        <th>Nama Modul</th>
                    ...
                    @endphp
                        ...
                        <tr style="text-align: center">
                            ...
                            <td>{{ $end }}</td>
                            <td>{{ $doneInterval }}</td>
                            @foreach ($r->score_values as $k => $s)
                                <td class="{{$s->is_passed ? 'text-green' : 'text-red'}}">{{$s->score ?? ''}}</td>
                            @endforeach
                            <td class="{{$r->status ? 'text-green' : 'text-red'}}">{{$r->total_score ?? ''}}</td>
                            <td class="{{$r->status ? 'text-green' : 'text-red'}}">{{$r->status_text ?? ''}}</td>
                            <td>{{$r->repeat ?? ''}} kali</td>
                        </tr>
                    @endforeach
        ...
        <table class="module custom-table">
            <tr class="head">
                @foreach (($report->subject->score_values ?? []) as $key => $value)
                    <th>Rata-Rata {{strtoupper($key)}}</th>
                @endforeach
                <th>Rata-Rata nilai akhir</th>
            </tr>
            <tr style="font-size: 1.5rem">
                @foreach (($report->subject->score_values ?? []) as $v)
                    <th>{{$v->average_score ?? 0}}</th>
                @endforeach
        ...
        <section class="description">
            <div>
                <ul>
                    @foreach (($report->subject->score_values ?? [])  as $k => $v)
                        <li>
                            <b>Rata-Rata {{$k}} = </b> Total Nilai {{$k}} / Jumlah Modul Diterima
                        </li>
                    @endforeach
                    <li>
                        <b>Rata-Rata Nilai Akhir = </b> Total Nilai Nilai Akhir / Jumlah Modul Diterima
                    </li>
                </ul>
            </div>
        </section>
