...
@section('title', 'Add Additional Controls')
...
<section id="input-sizing">
    <div class="card">
        <div class="card-body">
            <div>
              <form action="{{ route('additional-control-sso.store') }}" id="additional-control" method="POST">
                @csrf
                    ...
                    <div class="form-group">
                        ...
                        <select name="username" id="user_role_id" class="form-control select2 @error('username') is-invalid @enderror" placeholder="Username" required>
                          ...
                          @foreach($userRole as $data)
                            <option value="{{ $data->id }}">{{ $data->users->name }} ({{ $data->users->email }}) - {{ $data->role_name }}</option>
                          @endforeach
                        </select>
                      ...
                      <div class="form-group">
                        ...
                        <select name="acl" id="acl" class="form-control select2 @error('acl') is-invalid @enderror" placeholder="ACL" required>
                          <option value="">Pilih Application Control List</option>
