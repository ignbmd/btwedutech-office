      ...
      <div class="card">
        <div class="card-header border-bottom p-1">
          ...
          <div class="text-right">
            <div class="d-inline-flex">
              <a href="/admin-sso/applications/tambah" class="btn btn-primary">
        ...
        <div class="table-responsive">
          <table class="datatables-basic table">
            ...
            <tbody>
              @forelse($application ?? [] as $index => $data)
              <tr>
                <td>{{ $index + 1 }}</td>
                <td>{{ $data->application_name }}</td>
                <td>{{ $data->application_url }}</td>
                ...
                <td>
                    <div class="btn-group">
                      ...
                      <div class="dropdown-menu">
                        <a class="dropdown-item w-100" href="/admin-sso/applications/edit/{{ $data->id }}">
  ...
  $(function() {
    ...
    function showConfirmationPopUp(id) {
      Swal.fire({
        ...
        if(result.isConfirmed) deleteApplications(id);
      });
    }


    function deleteApplications(ssoId) {
        const token = $("meta[name='csrf-token']").attr("content");

         $.ajax({
           url: `/api/sso/application/${ssoId}`,
           type: "DELETE",
           data : { "_token": token },
    ...
    function showDeleteSuccessToast(data) {
      toastr.success(data?.data.message, "Hapus application berhasil", {
        timeOut: 1500,
        closeButton: true,
        tapToDismiss: true,
        preventDuplicates: true,
        onHidden() {
          window.location.href = "/admin-sso/applications";
        },
      })
    }
