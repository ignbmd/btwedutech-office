<div class="form-group">
  ...
  <input type="hidden" name="current_coin_amount" value="{{ $product->coin_amount ?? null}}" />
  <input type="hidden" name="current_max_discount_amount" value="{{ $product->max_discount->amount }}" />
  <input type="hidden" id="current_parent_product_id" name="current_parent_product_id" value="{{ $product->parent_product_id ?? null }}" />
  <input type="hidden" id="current_product_type" name="current_product_type" value="{{ $product->type }}" />
  <input type="hidden" id="currentLegacyId" name="current_legacy_id" value="{{ $product->legacy_id }}">
</div>
<div class="form-group">
...
<div class="form-group">
  ...
  <select id="type" name="type" class="form-control hide-search">
    <option value="ONLINE_PRODUCT" @if($product->type == 'ONLINE_PRODUCT') selected @endif>Product Online</option>
    <option value="OFFLINE_PRODUCT" @if($product->type == 'OFFLINE_PRODUCT') selected @endif>Product Offline</option>
    <option value="COIN_PRODUCT" @if($product->type == 'COIN_PRODUCT') selected @endif>Product Koin</option>
    <option value="COIN_CURRENCY" @if($product->type == 'COIN_CURRENCY') selected @endif>Mata Uang Koin</option>
    <option value="ONLINE_PRODUCT_IOS" @if($product->type == 'ONLINE_PRODUCT_IOS') selected @endif>Product Online IOS
    </option>
    <option value="ONLINE_PRODUCT_ANDROID" @if($product->type == 'ONLINE_PRODUCT_ANDROID') selected @endif>Product
      Online Android</option>
  </select>
</div>
<div class="form-group">
...
<div class="form-group">
  ...
  <select id="program" name="program" class="select2 form-control form-control-lg hide-search">
    ...
    <option value="{{$key}}" @if ($product->program == $key) selected @endif>{{$programs[$key]}}</option>
    @endforeach
  </select>
</div>
<div class="form-group" id="legacy-container">
  <label class="form-label" for="legacy-id">
    Legacy Id
    <span class="badge badge-warning" id="editLegacyProductBadge"></span>
  </label>
  <select id="legacy-id" name="legacy_id" class="select2 form-control form-control-lg" required>
