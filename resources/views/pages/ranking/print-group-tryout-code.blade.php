        ...
        @foreach($ranking as $index => $data)
          <tr>
            ...
            @foreach($data->score ?? [] as $scoreIndex => $scoreValue)
              <td style="text-align: center;" class="{{ $scoreValue->score >= $scoreValue->passing_grade ? 'text-green' : 'text-red' }}">{{ $scoreValue->score }}</td>
            @endforeach
            <td style="text-align: center;" class="{{ $data->status ? 'text-green' : 'text-red' }}">{{ $data->total_score }}</td>
            <td style="text-align: center;" class="{{ $data->status ? 'text-green' : 'text-red' }}">{{ $data->status ? "LULUS" : "TIDAK LULUS" }}</td>
          </tr>
        @endforeach
      </table>
