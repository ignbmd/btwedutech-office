...
<head>
    ...
    <title>{{ 'Laporan Rekapitulasi Perkembangan Siswa Kelas BTW Edutech - ' . $classTitle }}</title>
...
<body>
    ...
    <table class="header">
        <tr>
            <td class="title">
                Laporan Rekapitulasi Perkembangan Siswa Kelas BTW Edutech
            </td>
                    ...
                    <tr class="row" style="text-align: center">
                        ...
                        @if (count($class) > 1)
                            <td class="{{ isset($orderBy) && $orderBy == 2 ? 'bg-highlight' : '' }}">
                                {{ $studentClasses[$result->smartbtw_id] }}</td>
                        @endif
                        ...
                        <td class="{{ isset($orderBy) && $orderBy == "received-module" ? 'bg-highlight' : '' }}">
                            {{ $result->summary->owned }}</td>
                        <td class="{{ isset($orderBy) && $orderBy == "done-module" ? 'bg-highlight' : '' }}">
                            {{ $result->summary->done }}</td>
                        <td class="{{ isset($orderBy) && $orderBy == "done-percent" ? 'bg-highlight' : '' }}">
                            {{ round($result->summary->done_percent, 0) }}%</td>

                        @if($program !== "tps")
                          <td class="{{ isset($orderBy) && $orderBy == "passed-percent" ? 'bg-highlight' : '' }}">
                              {{ round($result->summary->passed_percent, 0) }}%</td>
                          <td
                            class="{{ $result->summary->score_values && $result->summary->score_values->TWK->average_score > $nilaiMinimal->TWK
                                ? 'text-green'
                                : 'text-red' }} {{ isset($orderBy) && $orderBy == "average-0" ? 'bg-highlight' : '' }}">
                            {{ $result->summary->score_values->TWK->average_score }}
                          </td>
                          <td
                            class="{{ $result->summary->score_values && $result->summary->score_values->TIU->average_score > $nilaiMinimal->TIU
                                ? 'text-green'
                                : 'text-red' }} {{ isset($orderBy) && $orderBy == "average-1" ? 'bg-highlight' : '' }}">
                            {{ $result->summary->score_values->TIU->average_score }}
                          </td>
                          <td
                            class="{{ $result->summary->score_values && $result->summary->score_values->TKP->average_score > $nilaiMinimal->TKP
                                ? 'text-green'
                                : 'text-red' }} {{ isset($orderBy) && $orderBy == "average-2" ? 'bg-highlight' : '' }}">
                            {{ $result->summary->score_values->TKP->average_score }}
                          </td>
                        ...
                        @else
                          @foreach($scoreKeys as $index => $keys)
                            <td
                              ...
                              {{ $result->summary->score_values->{$keys}->average_score }}
                            </td>
                          @endforeach
                        ...
                        @if($program !== "tps")
                          <td
                              class="{{ $result->summary->score_values &&
                              $result->summary->score_values->TWK->average_score > $nilaiMinimal->TWK &&
                              $result->summary->score_values->TIU->average_score > $nilaiMinimal->TIU &&
                              $result->summary->score_values->TKP->average_score > $nilaiMinimal->TKP
                                  ? 'text-green'
                        ...
                        <td class="{{ isset($orderBy) && $orderBy == "average-try" ? 'bg-highlight' : '' }}">
                            {{ $result->summary->report_average_try }}</td>
                        <td class="{{ isset($orderBy) && $orderBy == "repeat-total" ? 'bg-highlight' : '' }}">
                            {{ property_exists($result->summary, 'total')
                                ? ($result->summary->total > 0
                                    ? $result->summary->total . ' kali'
                                    : '0')
                                : '0' }}
                        </td>
                    </tr>
