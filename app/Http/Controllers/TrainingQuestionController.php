...
class TrainingQuestionController extends Controller
{
  ...
  public function showSubCategoryModules(Request $request, $test_type, $program, $question_category)
  {
    ...
    $programBreadcrumbs = [
      "skd" => [
        [
          "name" => "Modul Soal Latihan $testTypeTitle - $upperCasedQuestionCategory",
          "link" => null
        ]
      ],
      "utbk" => [
        [
          "name" => "Modul Soal Latihan $testTypeTitle - " . ucwords(str_replace('-', ' ', $question_category)),
          "link" => null
        ]
      ],
    ];
    ...
    return view('/pages/training-question/sub-category-modules', compact(
  ...
  public function showQuestions(Request $request, $test_type, $program, $question_category, $module_code)
  {
    ...
    $programBreadcrumbs = [
      "skd" => [
        [
          "name" => "Modul Soal Latihan $testTypeTitle  - $upperCasedQuestionCategory",
          "link" => "/soal-latihan/$test_type/$program/$question_category"
        ],
        [
    ...
    return view('/pages/training-question/question', compact(
  ...
  public function showExplanation(Request $request, $test_type, $program, $question_category, $module_code, $question_id)
  {
    ...
    $validModuleCodes = $test_type === "pre-test" ? $this->getPreTestModuleCodes() : $this->getPostTestModuleCodes();
    if(!in_array($module_code, $validModuleCodes)) {
      Log::error("Cannot get $test_type training module explanation data", [
        'user_id' => Auth::user()->id,
        'valid_module_codes' => $validModuleCodes,
        'current_office_path' => $request->getRequestUri(),
        'message' => 'Module code is not valid'
    ...
    if(!$edutechProgram) {
      Log::error("Cannot get $test_type training module explanation data", [
        'success' => false,
        'user_id' => Auth::user()->id,
        'message' => 'Any other program except ptk and ptn is not supported at the moment',
        'current_office_path' => $request->getRequestUri()
      ]);
    ...
    $iframeLink = "$explanationHost/pembahasan/" . strtolower($edutechProgram) . "/" . $encryptedQuestionId . "?mid=$encryptedUserId&mnm=$encryptedUserName&track=comment";

    return view('/pages/training-question/explanation', compact(
      'iframeLink', 'question_id'
    ));
  }

  public function showComments(Request $request, $test_type, $program, $question_category, $module_code, $question_id)
  {
    // Validate test type route param value
    $validTestTypes = ["pre-test", "post-test"];
    $lowerCasedTestType = strtolower($test_type);
    ...
    $validModuleCodes = $test_type === "pre-test" ? $this->getPreTestModuleCodes() : $this->getPostTestModuleCodes();
    if(!in_array($module_code, $validModuleCodes)) {
      Log::error("Cannot get $test_type training modules question comments data", [
        'user_id' => Auth::user()->id,
        'valid_module_codes' => $validModuleCodes,
        'current_office_path' => $request->getRequestUri(),
        'message' => 'Module code is not valid'
    ...
    $questionResponse = $this->examQuestionService->getById($question_id);
    $questionBody = json_decode($questionResponse->body())?->data ?? null;
    if(!$questionBody) {
      Log::error("Cannot get $test_type training modules question comments data", [
        'user_id' => Auth::user()->id,
        'current_office_path' => $request->getRequestUri(),
    ...
    if($questionBody->program !== $program) {
      Log::error("Cannot get $test_type training modules question comments data", [
        'user_id' => Auth::user()->id,
        'current_office_path' => $request->getRequestUri(),
        'message' => "Question data program doesn't match"
      ]);
      abort(404);
    }

    if($program === "skd" && $questionBody->sub_category_questions_id !== (int)$subCategoryId) {
      Log::error("Cannot get $test_type training modules question comments data", [
        'user_id' => Auth::user()->id,
        'current_office_path' => $request->getRequestUri(),
        'question_sub_category_id' => $questionBody->sub_category_questions_id,
        'inputted_sub_category_id' => $subCategoryId,
        'message' => "Question data sub category id doesn't match"
      ]);
      abort(404);
    }

    $questionCommentsResponse = $this->examQuestionService->getQuestionComments($question_id);
    ...
    $programBreadcrumbs = [
      "skd" => [
        ...
        [
          "name" => "Soal " . $subCategoryModule?->title ?? "",
          "link" => "/soal-latihan/$test_type/$program/$question_category/$module_code"
        ],
        [
          "name" => "Komentar Soal #$question_id",
          "link" => null
        ]
      ],
      "utbk" => [
    ...
    $breadcrumbs = $programBreadcrumbs[$program];
    return view('/pages/training-question/comment', compact(
      'breadcrumbs', 'testTypeTitle', 'upperCasedQuestionCategory',
      'subQuestionCategory', 'test_type', 'program', 'question_category', 'module_code',
      'subCategoryModule', 'questionComments', 'question_id'
    ));
  }
